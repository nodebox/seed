...
Path is the most versatile and complex shape. It can generate straight lines, quadratic béziers, cubic béziers, and arcs. It uses an efficient description that uses single-letter commands and coordinates, e.g. `M20 40` means "move to coordinate 20,40". Coordinates can be specified in both *absolute* and *relative* mode using uppercase and lowercase letters, respectively.
...
Since we have to do this in two steps it difficult to generate a number of random gradients and have PCG refer to them. We're looking into a solution for this, using some sort of memory function so the system knows which gradients it has generated.
